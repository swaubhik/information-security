...
function DoVigenere(et, key1, key2, abc, dir, vigtype, altluabc) {
  ...
  for (i = 0; i < et.length; i++) {
    let = et.charAt(i);
    letinabc = abc.indexOf(let);
    if (letinabc < 0) {
      dt += let;
      et = et.substring(0, i) + et.substring(i + 1, et.length);
      i--;
    } else {
      if (dir == "E") {
        dt += lu.charAt(
          (key1.indexOf(let) +
            key1.length +
            key1.indexOf(key2.charAt(i % key2.length))) %
            key1.length
        );
      }
      if (dir == "D") {
...
function DoFreqCnt(x, abc) {
  ...
  pos = x.indexOf(" ");
  while (pos > -1) {
    x = x.substring(0, pos) + x.substring(pos + 1, x.length);
    pos = x.indexOf(" ");
  }
  x = x.toUpperCase();
  freqs = "";
...
function DoRowColumnTranspose(et, rowcol, jump, startrow) {
  dt = "";
  if (et == "" || rowcol == "" || jump == "" || startrow == "") {
    dt = "You must supply all values";
    return dt;
  }
  maxrow = eval(rowcol.substring(0, rowcol.indexOf(",")));
  maxcol = eval(rowcol.substring(rowcol.indexOf(",") + 1, rowcol.length));
  jump = eval(jump);
  startrow = eval(startrow);
  if (startrow > maxrow) {
    dt = "Start Row must be <= Max Rows";
    return dt;
  }
  lin = new Array("");
  for (i = 0; i < maxrow; i++) {
    lin[i] = et.substring(maxcol * i, maxcol * (i + 1));
  }
  row = startrow - 1;
  col = maxcol - 1; //starting point
...
function DoModTranspose(et, startlet, jumpinc, modulus) {
  dt = "";
  if (et == "" || startlet == "" || jumpinc == "" || modulus == "") {
    dt = "You must supply all values";
    return dt;
  }
  startlet = eval(startlet) - 1;
  jumpinc = eval(jumpinc);
  modulus = eval(modulus);
  if (startlet > modulus) {
    dt = "startlet must be <= maxchar";
    return dt;
  }
  et = escape(et);
  pos = et.indexOf("%0D");
  while (pos > -1) {
    et = et.substring(0, pos) + et.substring(pos + 3, et.length);
    pos = et.indexOf("%0D");
  }
  pos = et.indexOf("%0A");
  while (pos > -1) {
  ...
  et = unescape(et);
  for (i = 0; i < modulus; i++) {
    dt += et.charAt((startlet + jumpinc * i) % modulus);
  }
  return dt;
}
...
function MakePlayfairSquare(abc, key1) {
  ...
  for (i = 0; i < 5; i++) {
    for (j = 0; j < 5; j++) row[i] += cyabc.charAt(5 * i + j);
  }
  sqr = "";
  for (i = 0; i < 5; i++) {
    sqr += row[i] + "\n";
